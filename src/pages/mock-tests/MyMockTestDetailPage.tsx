...
export function MyMockTestDetailPage() {
    ...
    try {
      ...
      const answersArray = questions.map(question => {
        ...
        return {
          ...
          timeSpent: Number(Math.round(questionTimeSpent / 1000))
        };
      });
      ...
      const submissionData = {
        ...
        correctCount: Number(correctAnswers),
  ...
  const startTest = async () => {
    if (!exam || !user?.id || !id) return;
    
    showLoading('Loading questions...');
    try {
      // Fetch exam details for this user to get questionIds
      const details = await api.get(`/exams/optimized/user/${user.id}/${id}/details?currentUserId=${user.id}`);
      const data = details?.data?.item || details?.data;
      const qIds: number[] = (data?.questionIds || data?.questions || []) as number[];
      
      if (qIds?.length) {
        // Fetch all questions
        const results = await Promise.all(qIds.map((qid) => api.get(`/questions/${qid}`)));
        const mapped: Question[] = results.map(r => {
          const q = r.data;
          const optionsArr = Array.isArray(q.options) ? q.options : (q.options ? Object.values(q.options) : []);
          return {
            id: String(q.id),
            contentQuestion: q.content || q.contentQuestion,
            options: optionsArr?.map((o: any) => (typeof o === 'string' ? o : o?.content || String(o))) || [],
            correctAnswer: q.correctAnswer,
            explanation: q.explanation,
            imageUrl: q.image || q.imageUrl,
            formula: q.formula,
            difficultyLevel: q.difficultyLevel || q.difficultyLevelId?.toString(),
    ...
    try {
      ...
      if (data) {
        const mappedExam: Exam = {
          ...
          lesson: data.lessonName || 'Unknown Lesson',
          duration: data.duration || 30,
          totalQuestions: data.totalQuestions || 0,
          examTypeName: data.examTypeName || 'Mock Test'
        };
  ...
  }, [id, user?.id, showLoading, hideLoading]);
        ...
        <div className="text-center">
          ...
          <div className="flex gap-4 justify-center">
            ...
            <Button asChild variant="outline">
              <Link to="/my-mock-tests">Back to My Mock Tests</Link>
            </Button>
          </div>
        </div>
      </div>
